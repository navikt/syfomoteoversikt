...
import * as moterActions from "../data/moter/moter_actions";
import * as brukerActions from "../data/bruker/bruker_actions";
...
export class Moteside extends Component {
  constructor(props) {
    super(props);
    if (
      props.harOvertattMoter ||
      (!props.henterMoterBool &&
        !props.hentMoterFeiletBool &&
        props.moter.length === 0)
    ) {
      props.hentMoter();
    }
  }
...
const EnhetensMoteContainer = connect(
  mapStateToProps,
  Object.assign({}, virksomhetActions, brukerActions, moterActions, fnrActions)
)(Moteside);
